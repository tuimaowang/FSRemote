...
enum class FakerInputKeyboardUpdate {
    ...
    Unsupported,
    Rollover,
}; // wjy: 调用方必须区分状态变化、按键重复、未映射按键和六键上限，禁止把失败误当成有效 HID 报告。
...
} // wjy: 通用 Ctrl/Shift/Alt 按现有 Viewer 语义落到左侧位，低级钩子能提供左右 VK 时仍保留真实方向。

constexpr std::uint8_t fakerInputUsageForVirtualKey(int virtualKey) noexcept
{
    if (virtualKey >= 'A' && virtualKey <= 'Z') {
        return static_cast<std::uint8_t>(0x04 + virtualKey - 'A'); // wjy: HID 键盘字母 usage 从 A=0x04 连续排列到 Z=0x1D。
    }
    if (virtualKey >= '1' && virtualKey <= '9') {
        return static_cast<std::uint8_t>(0x1E + virtualKey - '1'); // wjy: 主键盘数字 1..9 对应 0x1E..0x26。
    }
    ...
    default: return 0; // wjy: 浏览器、音量等 Consumer Control 键不属于当前键盘 collection，继续交给系统 SendInput 兼容处理。
    }
}
...
public:
    FakerInputKeyboardUpdate update(int virtualKey, bool down)
    {
        ...
        if (modifier == 0 && usage == 0) {
            return FakerInputKeyboardUpdate::Unsupported; // wjy: 无法编码的 VK 不进入驱动状态，调用方可对该键单独保留 SendInput。
        }

        const auto existing = std::find(pressed_virtual_keys_.begin(), pressed_virtual_keys_.end(), virtualKey);
        if (down) {
            if (existing != pressed_virtual_keys_.end()) return FakerInputKeyboardUpdate::Unchanged; // wjy: 相同 HID 快照不重复写管道，按住重复由目标 Windows 键盘类驱动产生。
            if (usage != 0 && ordinaryKeyCount() >= 6) {
                return FakerInputKeyboardUpdate::Rollover; // wjy: boot-keyboard 只能表达六个普通键，第七键不污染已按住的六键快照。
            }
            pressed_virtual_keys_.push_back(virtualKey); // wjy: 保留按下顺序使每次快照稳定，抬起一个键不会无意义重排其余 usage。
            return FakerInputKeyboardUpdate::Changed;
        }

        if (existing == pressed_virtual_keys_.end()) return FakerInputKeyboardUpdate::Unchanged;
        pressed_virtual_keys_.erase(existing); // wjy: 只有真实存在于驱动快照中的键才会产生新的抬键报告。
        return FakerInputKeyboardUpdate::Changed;
    }
    ...
    FakerInputKeyboardReport report() const noexcept
    {
        ...
        for (const int virtualKey : pressed_virtual_keys_) {
            const std::uint8_t modifier = fakerInputModifierBit(virtualKey);
            if (modifier != 0) {
                result.modifiers |= modifier; // wjy: 左右修饰键合并到标准八位掩码，不占六个普通键槽位。
                continue;
            }
            const std::uint8_t usage = fakerInputUsageForVirtualKey(virtualKey);
    ...
    {
        return pressed_virtual_keys_; // wjy: Bridge 断管时按现有逻辑状态重建系统 key-down，避免仍按住的移动键突然丢失。
    }

    void clear() noexcept
    {
        pressed_virtual_keys_.clear(); // wjy: 后端完成 release-all 后清空本地镜像，下一次按下从干净状态重新建立。
    }
