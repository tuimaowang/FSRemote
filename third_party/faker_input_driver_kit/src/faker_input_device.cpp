...
// =====wjy====
//  保留经过现有 FSRemote 验证的 FakerInput 设备枚举与报告写入实现，避免接收方重新猜测 VID/PID、Usage 和报告布局。
// ===end====
...
FakerInputDevice::FakerInputDevice(FakerInputDevice&& other) noexcept
    : handle_(std::exchange(other.handle_, INVALID_HANDLE_VALUE)),
      ...
      // =====wjy====
      absolute_buttons_(std::exchange(other.absolute_buttons_, std::uint8_t{0})), // wjy: 移交句柄时同步转移绝对按钮镜像，旧对象不再承担释放责任。
      absolute_x_(std::exchange(other.absolute_x_, std::uint16_t{0})),
      absolute_y_(std::exchange(other.absolute_y_, std::uint16_t{0}))
      // ===end====
{}
...
FakerInputDevice& FakerInputDevice::operator=(FakerInputDevice&& other) noexcept {
    if (this != &other) {
        ...
        // =====wjy====
        absolute_buttons_ = std::exchange(other.absolute_buttons_, std::uint8_t{0}); // wjy: 移动赋值保持设备句柄与绝对鼠标状态归属于同一对象。
        absolute_x_ = std::exchange(other.absolute_x_, std::uint16_t{0});
        absolute_y_ = std::exchange(other.absolute_y_, std::uint16_t{0});
        // ===end====
    }
...
bool FakerInputDevice::send_absolute_mouse(
    ...
    if (!send_inner_report(report, error)) {
        return false; // wjy: 写入失败时保留上一份已确认状态，release_all 只释放驱动真实接收过的按钮。
    }
    absolute_buttons_ = buttons; // wjy: 成功后提交完整按钮快照；零按钮表示后续清理无需再发送绝对鼠标报告。
    absolute_x_ = x; // wjy: 若按钮仍按住，清理时在相同坐标抬起，禁止释放动作制造额外位移。
    absolute_y_ = y;
    return true;
    // ===end====
}
...
bool FakerInputDevice::release_all(DeviceError* error) const {
    ...
    // =====wjy====
    if (absolute_buttons_ != 0) { // wjy: 纯键盘或相对鼠标会话从未持有绝对按钮时禁止发送绝对报告，消除结束测试后的大幅光标跳动。
